...
final class RequestToString {
	...
	static String requestToString(ContentCachingRequestWrapper request) {
		...
		for (Map.Entry<String, List<String>> headerEntries : new ServletServerHttpRequest(request).getHeaders()
				.entrySet()) {
			for (String headerValue : headerEntries.getValue()) {
				toString.append("> ").append(headerEntries.getKey()).append(": ").append(headerValue)

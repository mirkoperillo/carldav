...
final class ResponseToString {
	...
	static String responseToString(ContentCachingResponseWrapper response) throws IOException {
		...
		for (String headerName : response.getHeaderNames()) {
			toString.append("< ").append(headerName).append(": ").append(response.getHeader(headerName))
		...
		IOUtils.copy(new ByteArrayInputStream(response.getContentAsByteArray()), writer,

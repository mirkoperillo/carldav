...
/**
 * <p>
 * Models a URI pattern such that candidate URIs can be matched against the
 * template to extract interesting information from them.
 * </p>
 * <p>
 * A URI pattern looks like
 * <code>/collection/{uid}/{projection}?/{format}?/*</code>. Each path segment
 * can be either a literal or a variable (the latter enclosed in curly braces}.
 * A segment can be further denoted as optional, in which case the segment is
 * trailed by a question mark. A final segment of <code>*</code> indicates that
 * the remainder of the candidate URI after the previous segment is matched.
 * </p>
 * <p>
...
public class UriTemplate {
	 ...
	 * @throws IllegalArgumentException if more or fewer values are provided than
	 *                                  are needed by the template or if a null
	 *                                  value is provided for a mandatory variable
	 */
	 ...
	 * optionally escaped.
	 *
	 * @param escape a flag determining whether or not bound variables are to be
	 *               escaped
	 * @param values the unescaped values to be bound
	 * @return a URI with variables replaced by bound values
	 * @throws IllegalArgumentException if more or fewer values are provided than
	 ...
	 * algorithm as {@link #bindAbsolute(boolean, String, String...)}. Bound values
	 * are escaped.
	 *
	 * @param escape a flag determining whether or not bound variables are to be
	 *               escaped
	...
	/**
	 * Generates a absolute URI relative to <code>base</code> by replacing the
	 * variable segments of the template with the provided values. All literal
	 * segments, optional or no, are always included. Values are bound into the
	 * template in the order in which they are provided. If a value is not provided
	 * for an optional variable segment, the segment is not included. Bound values
	...
	/**
	 * <p>
	 * Matches an escaped candidate uri-path against the template using the same
	 * algorithm as {@link match(boolean, String)}. Returns a <code>Match</code>
	 * instance containing the names and values of all variables found in the
	 * uri-path as specified by the template.
	 * </p>
	 *
	 * @param path the candidate uri-path
	 * @return a <code>Match</code>, or <code>null</code> if the path did not
	 *         successfully match
	 */
	...
	/**
	 * <p>
	 * Matches a possibly-escaped candidate uri-path against the template. Returns a
	 * <code>Match</code> instance containing the names and values of all variables
	 * found in the uri-path as specified by the template.
	 * </p>
	 * <p>
	 * Each literal segment in the template must match the corresponding segment in
	 * the uri-path unless the segment is optional. For each variable segment in the
	 * template, an entry is added to the <code>Match</code> to be returned; the
	 * entry key is the variable name from the template, and the entry value is the
	 * corresponding (unescaped) token from the uri-path. If the template includes
	 * an "all" segment, a match entry with key <code>*</code> is also included
	 * containing the remainder of the uri-path after the last matching segment.
	 * </p>
	 *

...
public class DavCollectionBase extends DavResourceBase implements WebDavResource, DavCollection {
	...
	public DavCollectionBase(CollectionItem collection, DavResourceLocator locator, DavResourceFactory factory)
	...
	protected WebDavResource memberToResource(Item item) throws CosmoDavException {
		...
		var locator = getResourceLocator().getFactory().createResourceLocatorByPath(getResourceLocator().getContext(),
				path);
	...
	protected WebDavResource collectionToResource(CollectionItem hibItem) {
		...
		DavResourceLocator locator = getResourceLocator().getFactory()
				.createResourceLocatorByPath(getResourceLocator().getContext(), path);
	...
	protected WebDavResource memberToResource(String uri) throws CosmoDavException {
		var locator = getResourceLocator().getFactory().createResourceLocatorByUri(getResourceLocator().getContext(),
				uri);
	...
	public void writeBody(final HttpServletResponse response) throws IOException {
		try (var writer = new PrintWriter(new OutputStreamWriter(response.getOutputStream(), StandardCharsets.UTF_8))) {
			...
			var principalLocator = getResourceLocator().getFactory()
					.createPrincipalLocator(getResourceLocator().getContext(), getUsername());
			writer.write("<a href=\"");
			...
			writer.write("<p>\n");
			if (!isHomeCollection()) {
				var homeLocator = getResourceLocator().getFactory().createHomeLocator(getResourceLocator().getContext(),
						getUsername());
				writer.write("<a href=\"");

...
public class BaseProvider implements DavConstants {
	...
	public void get(HttpServletRequest request, HttpServletResponse response, WebDavResource resource)
			throws CosmoDavException, IOException {
		spool(request, response, resource, true);
	}

	public void head(HttpServletRequest request, HttpServletResponse response, WebDavResource resource)
			throws CosmoDavException, IOException {
		spool(request, response, resource, false);
	}

	public void propfind(HttpServletRequest request, HttpServletResponse response, WebDavResource resource)
			throws CosmoDavException, IOException {
		if (!resource.exists()) {
	...
	public void put(HttpServletRequest request, HttpServletResponse response, WebDavResource content)
			throws CosmoDavException, IOException {
		...
		if (!(content instanceof DavItemResourceBase)) {
			throw new IllegalArgumentException(
					"Expected type for 'content' is: [" + DavItemResourceBase.class.getName() + "]");
		}
	...
	public void delete(HttpServletRequest request, HttpServletResponse response, WebDavResource resource)
			throws CosmoDavException, IOException {
		if (!resource.exists()) {
		...
		if (resource instanceof DavCollectionBase collection) {
			...
			throw new IllegalArgumentException(String.format("Expected 'member' as instance of: [%s or %s]",
					DavItemResourceBase.class.getName(), DavCollectionBase.class.getName()));
		}

		response.setStatus(204);
	}

	public void report(HttpServletRequest request, HttpServletResponse response, WebDavResource resource)
			throws CosmoDavException, IOException {
		if (!resource.exists()) {
			throw new NotFoundException();
		}
	...
	protected void spool(HttpServletRequest request, HttpServletResponse response, WebDavResource resource,
	...
	private void parsePropFindRequest(final HttpServletRequest request) throws CosmoDavException {
		...
		throw new BadRequestException("Expected one of " + XML_PROP + ", " + XML_PROPNAME + ", or " + XML_ALLPROP
				+ " as child of " + XML_PROPFIND);
	}
	...
	private Document getRequestDocument(final HttpServletRequest request) {
		...
		/*
		 * Don't attempt to parse the body if the content length header is 0. NOTE: a
		 * value of -1 indicates that the length is unknown, thus we have to parse the
		 * body. Note that http1.1 request using chunked transfer coding will therefore

...
public class StandardResourceFactory implements DavResourceFactory, ExtendedDavConstants {
	...
	private WebDavResource createUnknownResource(DavResourceLocator locator) {
		...
		if (collectionName != null && itemUid != null) {
			var userItem = itemRepository.findByOwnerEmailAndCollectionNameAndName(securityManager.getUsername(),
					collectionName, locator.itemUid());
			if (userItem == null) {
				return null;
			}
		...
		if (collectionName != null) {
			var userCollection = collectionRepository.findByOwnerEmailAndName(securityManager.getUsername(),
					collectionName);

...
public class StampFilter {
	...
	/**
	 * Used in time-range filtering. If set, and there is a time-range to filter on,
	 * the timezone will be used in comparing floating times. If null, the server
	 * time-zone will be used in determining if floating time values match.
	 * 
	...
	private void updateFloatingTimes() {
		...
		if (tz != null) {
			// for some reason ical4j's TimeZone for a valid ID (i.e. Europe/Bucharest) has
			// raw offset 0

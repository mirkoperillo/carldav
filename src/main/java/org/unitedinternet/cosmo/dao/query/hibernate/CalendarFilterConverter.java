...
public class CalendarFilterConverter {
	...
	/**
	 * Tranlsate CalendarFilter to an equivalent ItemFilter. For now, only the basic
	 * CalendarFilter is supported, which is essentially a timerange filter. The
	 * majority of CalendarFilters will fall into this case. More cases will be
	 * supported as they are implemented.
	 *
	...
	private ItemQuery handleCompFilter(ComponentFilter compFilter) {
		try {
			return handleEventCompFilter(compFilter,
					new StampFilter(Item.Type.valueOf(compFilter.getName().toUpperCase(Locale.ENGLISH))));
		} catch (Exception e) {
	...
	private ItemQuery handlePropertyFilter(String propertyName, PropertyFilter propFilter) {
		...
		return new ItemQuery().propertyLike(propertyName, textMatch.getValue(), textMatch.isCaseless(),
				textMatch.isNegateCondition());
	}

...
/**
 * Utility class that contains apis that that involve expanding recurring
 * components.
 */
public class RecurrenceExpander {
	...
	/**
	 * Return start and end Date that represent the start of the first occurrence of
	 * a recurring component and the end of the last occurence. If the recurring
	 * component has no end(infinite recurring event), then no end date will be
	 * returned.
	 * 
	...
	/**
	 * Return a start and end Date that represents the start of the first occurence
	 * of a recurring component and the end of the last occurence. If the recurring
	 * component has no end(infinite recurring event), then no end date will be
	 * returned.
	 * 
	...
	public Date[] calculateRecurrenceRange(Component comp, List<Component> modifications) {
		...
		for (Iterator i = rRules.iterator(); i.hasNext();) {
			...
			if (startDates.size() > 0) {
				...
				Date endDate = org.unitedinternet.cosmo.calendar.util.Dates.getInstance(duration.getTime(lastStart),
						start);
	 ...
	 * @return InstanceList containing all occurences of recurring event during time
	 *         range
	 */
	 ...
	 * @param timezone      Optional timezone to use for floating dates. If null,
	...
	public boolean isOccurrence(Calendar calendar, Date occurrence) {
		...
		Date rangeEnd = org.unitedinternet.cosmo.calendar.util.Dates.getInstance(cal.getTime(), occurrence);

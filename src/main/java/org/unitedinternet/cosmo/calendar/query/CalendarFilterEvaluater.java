...
/**
 * Contains methods for determining if a Calendar matches a CalendarFilter.
 */
public class CalendarFilterEvaluater {
	...
	private boolean evaluateComps(ComponentList components, ComponentFilter filter) {
		...
		/*
		 * The CALDAV:comp-filter XML element is empty and the calendar component type
		 * specified by the "name" attribute exists in the current scope;
		 */
		...
		/*
		 * The CALDAV:comp-filter XML element contains a CALDAV:is-not- defined XML
		 * element and the calendar object or calendar component type specified by the
		...
		/*
		 * The CALDAV:comp-filter XML element contains a CALDAV:time-range XML element
		 * and at least one recurrence instance in the targeted calendar component is
		 * scheduled to overlap the specified time range, and all specified
		 * CALDAV:prop-filter and CALDAV:comp-filter child XML elements also match the
		 * targeted calendar component;
		 */
	...
	private boolean evaluate(Component component, PropertyFilter filter) {
		...
		/*
		 * The CALDAV:prop-filter XML element is empty and a property of the type
		 * specified by the "name" attribute exists in the enclosing calendar component;
		 */
		 ...
		 * element and no property of the type specified by the "name" attribute exists
		 * in the enclosing calendar component;
		 */
		if (filter.getIsNotDefinedFilter() != null) {
			PropertyList props = component.getProperties(filter.getName());
			return props.size() == 0;
		...
		/*
		 * The CALDAV:prop-filter XML element contains a CALDAV:time-range XML element
		 * and the property value overlaps the specified time range, and all specified
		 * CALDAV:param-filter child XML elements also match the targeted property;
		 */
	...
	private boolean evaulate(Property property, ParamFilter filter) {
		...
		/*
		 * The CALDAV:param-filter XML element is empty and a parameter of the type
		 * specified by the "name" attribute exists on the calendar property being
		 * examined;
		 */
		...
		/*
		 * The CALDAV:param-filter XML element contains a CALDAV:is-not- defined XML
		 * element and no parameter of the type specified by the "name" attribute exists
		 * on the calendar property being examined;
		 */
		if (filter.getIsNotDefinedFilter() != null) {
			ParameterList params = property.getParameters(filter.getName());
			return params.size() == 0;
	 ...
	 * corresponding component state specified in the table below is satisfied. Note
	 * that, as specified in [RFC2445], the DTSTART property is REQUIRED in the
	 * VEVENT component. The conditions depend on the presence of the DTEND and
	 * DURATION properties in the VEVENT component. Furthermore, the value of the
	 * DTEND property MUST be later in time than the value of the DTSTART property.
	 * The duration of a VEVENT component with no DTEND and DURATION properties is 1
	 * day (+P1D) when the DTSTART is a DATE value, and 0 seconds when the DTSTART
	 * is a DATE-TIME value.
	 * 
	 * +---------------------------------------------------------------+ | VEVENT
	 ...
	 * +-------------------------------------------------------+ | | | DURATION
	 * property value is greater than 0 seconds? | | | |
	 * +---------------------------------------------------+ | | | | DTSTART
	 ...
	 * +---+---+---+---+-----------------------------------------------+ | N | N | N
	 * | Y | (start <= DTSTART AND end > DTSTART) |
	 * +---+---+---+---+-----------------------------------------------+ | N | N | N
	 * | N | (start < DTSTART+P1D AND end > DTSTART) |
	 * +---+---+---+---+-----------------------------------------------+
	 */
	/**
	...
	/*
	 * A VJOURNAL component overlaps a given time range if the condition for the
	 * corresponding component state specified in the table below is satisfied. The
	 * conditions depend on the presence of the DTSTART property in the VJOURNAL
	 * component and on whether the DTSTART is a DATE-TIME or DATE value. The
	 * effective "duration" of a VJOURNAL component is 1 day (+P1D) when the DTSTART
	 * is a DATE value, and 0 seconds when the DTSTART is a DATE-TIME value.
	 * 
	 * +----------------------------------------------------+ | VJOURNAL has the
	 * DTSTART property? | | +------------------------------------------------+ | |
	 ...
	 * +---+---+--------------------------------------------+ | Y | Y | (start <=
	 * DTSTART) AND (end > DTSTART) |
	 * +---+---+--------------------------------------------+ | Y | N | (start <
	 * DTSTART+P1D) AND (end > DTSTART) |
	 * +---+---+--------------------------------------------+ | N | * | FALSE |
	 * +---+---+--------------------------------------------+
	 */
	/**
	...
	/*
	 * A VTODO component is said to overlap a given time range if the condition for
	 * the corresponding component state specified in the table below is satisfied.
	 * The conditions depend on the presence of the DTSTART, DURATION, DUE,
	 * COMPLETED, and CREATED properties in the VTODO component. Note that, as
	 * specified in [RFC2445], the DUE value MUST be a DATE-TIME value equal to or
	 * after the DTSTART value if specified.
	 * 
	 * +-------------------------------------------------------------------+ | VTODO
	 ...
	 * evaluate |
	 * +---+---+---+---+---+-----------------------------------------------+ | Y | Y
	 * | N | * | * | (start <= DTSTART+DURATION) AND | | | | | | | ((end > DTSTART)
	 * OR | | | | | | | (end >= DTSTART+DURATION)) |
	 * +---+---+---+---+---+-----------------------------------------------+ | Y | N
	 * | Y | * | * | ((start < DUE) OR (start <= DTSTART)) | | | | | | | AND | | | |
	 * | | | ((end > DTSTART) OR (end >= DUE)) |
	 * +---+---+---+---+---+-----------------------------------------------+ | Y | N
	 * | N | * | * | (start <= DTSTART) AND (end > DTSTART) |
	 * +---+---+---+---+---+-----------------------------------------------+ | N | N
	 * | Y | * | * | (start < DUE) AND (end >= DUE) |
	 * +---+---+---+---+---+-----------------------------------------------+ | N | N
	 * | N | Y | Y | ((start <= CREATED) OR (start <= COMPLETED))| | | | | | | AND |
	 * | | | | | | ((end >= CREATED) OR (end >= COMPLETED))|
	 * +---+---+---+---+---+-----------------------------------------------+ | N | N
	 * | N | Y | N | (start <= COMPLETED) AND (end >= COMPLETED) |
	 * +---+---+---+---+---+-----------------------------------------------+ | N | N
	 * | N | N | Y | (end > CREATED) |
	 * +---+---+---+---+---+-----------------------------------------------+ | N | N
	 * | N | N | N | TRUE |
	...
	/**
	 * Determine if VTODO overlaps timerange assuming the VTODO has no DTSTART,
	 * using the state table defined in RFC-4791 Sec 9.9.
	 * 
	 * @param vtodo  VToDo.
	...
	/**
	 * A VALARM component is said to overlap a given time range if the following
	 * condition holds:
	 * 
	 * (start <= trigger-time) AND (end > trigger-time)
	 * 
	 * A VALARM component can be defined such that it triggers repeatedly. Such a
	 * VALARM component is said to overlap a given time range if at least one of its
	 * triggers overlaps the time range.
	 * 
	 * @param comps  The component list.
	 * @param filter The time range filter.
	 * @return The result.

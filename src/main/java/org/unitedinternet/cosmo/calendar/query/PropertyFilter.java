 ...
 *
 * or: A property of the type specified by the "name" attribute does not exist,
 ...
 * param-filter*))>
 *
 * <!ATTLIST prop-filter name CDATA #REQUIRED> name value: a calendar property
 * name (e.g., "ATTENDEE")
 *
 */
		...
		while (i.hasNext()) {
			...
			if (ELEMENT_CALDAV_TIME_RANGE.equals(child.getLocalName())) {
				...
				if (timeRangeFilter != null) {
					throw new ParseException("CALDAV:prop-filter only one time-range or text-match element permitted",
							-1);
				}

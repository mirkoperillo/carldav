...
public class InstanceList extends TreeMap<String, Instance> {
		...
		for (var rDate : rDates) {
			...
			if (Value.PERIOD.equals(rdate.getParameters().getParameter(Parameter.VALUE))) {
				...
				for (var o : rdate.getDates()) {
					...
					var endDate = org.unitedinternet.cosmo.calendar.util.Dates.getInstance(duration.getTime(startDate),
							startDate);
	 ...
	 * @return true if the override component modifies instance list and false if
	 *         the override component has no effect on instance list
	 */
	public boolean addOverride(Component comp, Date rangeStart, Date rangeEnd) {
		...
		if (dtend == null) {
			...
			if (startValue.equals(Value.DATE_TIME)) {
				...
				// will be 1 day since its an all-day event
				if (dtend.before(dtstart)) {
					dtend = org.unitedinternet.cosmo.calendar.util.Dates
							.getInstance(new Dur(1, 0, 0, 0).getTime(dtstart), dtstart);
				}
			}
		}
		...
		var range = (Range) comp.getProperties().getProperty(Property.RECURRENCE_ID).getParameters()
	...
	private Date getEndDate(Component comp) {
		...
		if (dtEnd == null) {
			...
			if (duration != null) {
				dtEnd = new DtEnd(org.unitedinternet.cosmo.calendar.util.Dates
						.getInstance(duration.getDuration().getTime(dtStart), dtStart));
			}
		}
	...
	/**
	 * If the InstanceList is configured to convert all date/times to UTC, then
	 * convert the given Date instance into a UTC DateTime.
	 *
	 * @param date The date.
	 * @return The date.
	 */
	 ...
	 * startRange, but end after. For example if you have a daily recurring event at
	 * 8am lasting for an hour and your startRange is 8:01am, then you want to
	 * adjust the range back an hour to catch the instance that is already
	 * occurring.
	 *
	...
	/**
	 * Adjust endRange for Date instances. First convert the UTC endRange into a
	 * Date instance, then add a second
	 *
	...
	/**
	 * Adjust a floating time if a timezone is present. A floating time is initially
	 * created with the default system timezone. If a timezone if present, we need
	 * to adjust the floating time to be in specified timezone. This allows a server
	 * in the US to return floating times for a query made by someone whos timezone
	 * is in Australia. If no timezone is set for the InstanceList, then the system
	 * default timezone will be used in floating time calculations.
	 * <p/>
	 * What happens is a floating time will get converted into a date/time with a
	 * timezone. This is ok for comparison and recurrence generation purposes. Note
	 * that Instances will get indexed as a UTC date/time and for floating
	 * DateTimes, the the recurrenceId associated with the Instance loses its
	 * "floating" property.
	 *
	 * @param date The date.
	 * @return The date.

...
class ItemRepositoryTests extends IntegrationTestSupport {
	...
	void before() {
		...
		var collection = Optional.ofNullable(template.findById(1L, CollectionItem.class))
	...
	void displayNameCaseInsensitiveNot() {
		assertThat(itemRepository.findAll(new ItemQuery().propertyLike("displayName", "isplayna", true, true)))
				.isEmpty();
		assertThat(itemRepository.findAll(new ItemQuery().propertyLike("displayName", "isplayNa", true, true)))
				.isEmpty();
	}
